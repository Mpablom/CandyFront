...
export class ReservationListComponent implements OnInit, OnDestroy {
  ...
  loadReservations(): void {
    this.reservationService.getAllReservations().subscribe({
      next: (data: Reservation[]) => {
        ...
        this.deletedReservations = data.filter(
          (reservation) => reservation.deleted,
        );
  ...
  deleteReservation(id: number): void {
    this.reservationService.deleteReservation(id).subscribe({
      next: () => {
        const deletedReservation = this.reservations.find(
    ...
    this.reservationService.deletePastReservationsAndCustomers().subscribe({
      ...
      error: (error: any) => {
        ...
        this.showErrorSnackbar(
          'Error al eliminar reservas y clientes pasados.',
        );
      },
    });
  }
  ...
  groupReservationsByMonth(): void {
    const grouped = this.reservations.reduce(
      (acc, reservation) => {
        let date: Date;

        if (
          typeof reservation.reservationDate === 'string' &&
          reservation.reservationDate.includes('-')
        ) {
        ...
        let group = acc.find(
          (g) => g.month === monthGroup && g.year === yearGroup,
        );
        if (!group) {
    ...
    sortedGroups.forEach((group) => {
      group.reservations.sort((a, b) => {
        const parseDate = (res: Reservation): Date => {
          if (
            typeof res.reservationDate === 'string' &&
            res.reservationDate.includes('-')
          ) {
            const [year, month, day] = res.reservationDate.split('-');
            return new Date(+year, +month - 1, +day);
          } else if (
            typeof res.reservationDate === 'string' &&
            res.reservationDate.includes('/')
          ) {
            const [day, month, year] = res.reservationDate.split('/');
            return new Date(+year, +month - 1, +day);
          } else {
            return new Date(res.reservationDate);
          }
        };
    ...
    this.groupedReservations = sortedGroups;
  }
